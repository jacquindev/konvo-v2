import { EMBED_CONFIG, WIDGET_URL } from "./config";
...
(function () {
  ...
  if (currentScript) {
    ...
    const embedScript = Array.from(scripts).find((script) => script.hasAttribute("data-organization-id")) as HTMLScriptElement;
  ...
  if (!organizationId) {
    console.error("Konvo Widget: data-organization-id attribute is required.");
    return;
  }
  ...
  function render() {
    ...
    container.id = "konvo-widget-container";
